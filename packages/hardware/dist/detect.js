...
async function detectNvidiaGPUs() {
    ...
    try {
        ...
        const { stdout } = await execAsync('nvidia-smi --query-gpu=name,memory.total,driver_version,compute_cap --format=csv,noheader,nounits', { timeout: 10000 });
...
async function detectJetson() {
    try {
        ...
        if (stdout.toLowerCase().includes('jetson') || stdout.toLowerCase().includes('tegra')) {
            ...
            let model = 'Jetson';
            if (stdout.includes('Orin'))
                model = 'Jetson Orin';
            else if (stdout.includes('Xavier'))
                model = 'Jetson Xavier';
            else if (stdout.includes('Nano'))
                model = 'Jetson Nano';
            return {
                name: model,
                vendor: 'nvidia',
...
function determineTier(gpus, ramMb) {
...
function determineCapabilities(tier, gpus, ramMb) {
    const maxVram = Math.max(0, ...gpus.map((g) => g.vramMb));
...
export async function canRunLocalAI() {
