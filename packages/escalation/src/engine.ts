...
import {
  ...
  DecisionSource as DecisionSourceEnum,
} from './types.js';
...
export class EscalationEngine {
  private thresholds: Map<string, EscalationThresholds>;
  private decisionHistory: DecisionResult[] = [];
  private decisionsByStudent: Map<string, DecisionResult[]> = new Map();
  private situationPatterns: Map<string, SituationPattern> = new Map();
  private stats: DecisionStats;
  private enableLearning: boolean;

  constructor(config?: { enableLearning?: boolean }) {
  ...
  getThresholds(studentId: string): EscalationThresholds {
    if (!this.thresholds.has(studentId)) {
      this.thresholds.set(studentId, { ...DEFAULT_THRESHOLDS });
    }
  ...
  setPhaseThresholds(phase: LearningPhase, thresholds: Partial<EscalationThresholds>): void {
    ...
    this.thresholds.set(`phase:${phase}`, { ...DEFAULT_THRESHOLDS, ...thresholds });
  }

  /**
   * Get thresholds for a specific phase
   */
  private getThresholdsForContext(context: DecisionContext): EscalationThresholds {
  ...
  routeDecision(context: DecisionContext): EscalationDecision {
    ...
    if (isCriticalStakes || isTimeCritical) {
      decision = {
        source: DecisionSourceEnum.HUMAN,
    ...
    // Novel situations with high stakes -> Brain (or Human if very high)
      ...
      decision = {
        source: isCriticalStakes ? DecisionSourceEnum.HUMAN : DecisionSourceEnum.BRAIN,
        reason: EscalationReasonEnum.NOVEL_SITUATION,
    ...
    // Novel situations with low stakes -> Brain
      ...
      decision = {
        source: DecisionSourceEnum.BRAIN,
        reason: EscalationReasonEnum.NEW_CONCEPT,
        confidenceRequired: thresholds.brainMinConfidence,
        timeBudgetMs: context.urgencyMs,
        allowFallback: true,
        metadata: {},
    ...
    // High stakes but familiar -> Brain
      ...
      decision = {
        source: DecisionSourceEnum.BRAIN,
        reason: EscalationReasonEnum.HIGH_STAKES,
  ...
  private checkCriticalOverride(
    ...
    if (context.recentFailures >= 3) {
      return {
        source: DecisionSourceEnum.BRAIN,
        reason: EscalationReasonEnum.STUDENT_STUCK,
    ...
    if (context.progressRatio <= thresholds.resourceCriticalThreshold) {
      return {
        source: DecisionSourceEnum.BRAIN,
        reason: EscalationReasonEnum.SAFETY_CONCERN,
   ...
   */
  private isNovelSituation(context: DecisionContext, thresholds: EscalationThresholds): boolean {
  ...
  shouldEscalate(result: DecisionResult, context: DecisionContext): [boolean, EscalationReason | null] {
    ...
    if (result.source === DecisionSourceEnum.HUMAN) {
      return [false, null];
    }

    // Check confidence
    if (result.source === DecisionSourceEnum.BOT) {
      if (result.confidence < thresholds.botMinConfidence) {
        return [true, EscalationReasonEnum.LOW_CONFIDENCE];
      }
    } else if (result.source === DecisionSourceEnum.BRAIN) {
    ...
    if (context.stakes >= thresholds.criticalStakesThreshold) {
      if (result.source === DecisionSourceEnum.BOT) {
        return [true, EscalationReasonEnum.HIGH_STAKES];
      }
    }
  ...
  recordDecision(result: DecisionResult): void {
    ...
    const studentId = result.metadata?.studentId as string | undefined;
    if (studentId) {
    ...
    switch (result.source) {
      case DecisionSourceEnum.BOT:
        this.stats.botDecisions++;
        break;
      case DecisionSourceEnum.BRAIN:
        this.stats.brainDecisions++;
        break;
      case DecisionSourceEnum.HUMAN:
   ...
   */
  private updateThresholds(studentId: string, result: DecisionResult, success: boolean): void {
    const thresholds = this.getThresholds(studentId);

    // Adjust confidence thresholds based on success
    if (result.source === DecisionSourceEnum.BOT) {
      if (success) {
        thresholds.botMinConfidence = Math.max(
          0.5,
          thresholds.botMinConfidence - thresholds.confidenceBoostPerSuccess
        );
      } else {
        thresholds.botMinConfidence = Math.min(
          0.9,
          thresholds.botMinConfidence + thresholds.confidencePenaltyPerFailure
        );
      }
    } else if (result.source === DecisionSourceEnum.BRAIN) {
      if (success) {
        thresholds.brainMinConfidence = Math.max(
          0.3,
          thresholds.brainMinConfidence - thresholds.confidenceBoostPerSuccess
        );
      } else {
        thresholds.brainMinConfidence = Math.min(
          0.8,
          thresholds.brainMinConfidence + thresholds.confidencePenaltyPerFailure
        );
      }
    }
  }
  ...
  getStudentStats(studentId: string): StudentStats {
    ...
    const botDecisions = decisions.filter((d) => d.source === DecisionSourceEnum.BOT).length;
    const brainDecisions = decisions.filter((d) => d.source === DecisionSourceEnum.BRAIN).length;
    const humanDecisions = decisions.filter((d) => d.source === DecisionSourceEnum.HUMAN).length;
  ...
  getGlobalStats(): DecisionStats {
    ...
    const baselineCost = this.stats.totalDecisions * DEFAULT_COST_CONFIG.costHuman;
   ...
   */
  private createEmptyStats(): DecisionStats {
    return {
      totalDecisions: 0,
      botDecisions: 0,
      brainDecisions: 0,
   ...
   */
  clearOldPatterns(olderThanMs: number = 7 * 24 * 60 * 60 * 1000): void {
...
export function createContext(
  ...
  partialContext?: Partial<DecisionContext>
): DecisionContext {
  return {
    studentId,
    situationType,
...
export function createDecisionResult(
  ...
  action: string,
  confidence: number,
  partialResult?: Partial<DecisionResult>
): DecisionResult {
  return {
    decisionId: uuidv4(),
    source,
 ...
 */
export function estimateCost(source: DecisionSource): number {
  switch (source) {
    case DecisionSourceEnum.BOT:
      return DEFAULT_COST_CONFIG.costBot;
    case DecisionSourceEnum.BRAIN:
      return DEFAULT_COST_CONFIG.costBrain;
    case DecisionSourceEnum.HUMAN:
      return DEFAULT_COST_CONFIG.costHuman;
    default:
      return 0;
  }
}
